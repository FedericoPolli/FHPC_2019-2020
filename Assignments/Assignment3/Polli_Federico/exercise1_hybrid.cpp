...
int main(int argc, char* argv[]) {
  ...
  // calculate the number of threads to be spawned by each process 
...
#pragma omp for schedule(dynamic)
	for (std::size_t j=myid*n_x*n_y/numproc; j<(myid+1)*n_x*n_y/numproc; ++j)
	  {
	    ...
	    Matrix_255[j-myid*n_x*n_y/numproc]=check_bounded(c_x, c_y, I_max);    // Compute escape value for the point (c_x, c_y)   
	  }
	...
	min_time = CPU_TIME_th-mystart;
      ...
      /* If I am the master process, gather from all the other processes, in order of rank,
	 the chunks of the array containing the data necessary to plot the Mandelbrot set, then 
	 merge all the arrays together in a unique vector that is passed to the plot function. 
	 If I am another process, just send the data to the master.                             */
...
#pragma omp for schedule(dynamic)
	for (std::size_t j=myid*n_x*n_y/numproc; j<(myid+1)*n_x*n_y/numproc; ++j)
	  {
	    c_y=y_L+delta_y*(j/n_x);
	    c_x=x_L+delta_x*(j%n_x);	  
	    Matrix_65535[j-myid*n_x*n_y/numproc]=check_bounded(c_x, c_y, I_max);  // Compute escape value for the point (c_x, c_y) 	      
	  }
	...
	min_time = CPU_TIME_th-mystart;
      ...
      // Same as before: if master gather results and write to file, else send results to master
