	...
	for (std::size_t j=0; j<n_y; ++j)
	  {
	    ...
	    for (std::size_t k=0; k<n_x; ++k)
	      {
		c_x=x_L+k*delta_x;	  
		Matrix_255[k+j*n_x]=check_bounded(c_x, c_y, I_max);  // Compute escape value for the point (c_x, c_y) 
	      }
	  }
#endif
	...
	for (std::size_t j=0; j<n_y; ++j)
	  {
	    ...
	    for (std::size_t k=0; k<n_x; ++k)
	      {
		c_x=x_L+k*delta_x;	  
		Matrix_65535[k+j*n_x]=check_bounded(c_x, c_y, I_max);  // Compute escape value for the point (c_x, c_y) 
	      }
	  }
#endif
